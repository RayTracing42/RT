								...
								int i)
{
	...
	if (gt(t = plane_intersect(ray, e, obj, i), 0))
	{
		...
		if (gt(angle_between_vectors(tgl->vAB, dots_to_vect(tgl->dA, itmp)),
				tgl->aA) ||
			gt(angle_between_vectors(tgl->vBA, dots_to_vect(tgl->dB, itmp)),
				tgl->aB) ||
			gt(angle_between_vectors(tgl->vBC, dots_to_vect(tgl->dB, itmp)),
				tgl->aB))
			t = -1;
	}
	return (t);
...
static int		is_in_triangle(t_dot *i, t_object *obj)
{
	...
	{
		if (gt(angle_between_vectors(tgl->vAB, dots_to_vect(tgl->dA, *i)),
				tgl->aA) ||
			gt(angle_between_vectors(tgl->vBA, dots_to_vect(tgl->dB, *i)),
				tgl->aB) ||
			gt(angle_between_vectors(tgl->vBC, dots_to_vect(tgl->dB, *i)),
				tgl->aB))
			return (0);
		return (1);
	}
