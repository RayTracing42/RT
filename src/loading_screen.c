...
/*   Updated: 2018/02/23 11:30:35 by edescoin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_loadingbar	new_loading_bar()
{
	...
	if(!(tmp = SDL_LoadBMP("resources/loading.bmp")) ||
		!(lb.loadingtexture = SDL_CreateTextureFromSurface(get_sdl_core()->renderer, tmp)))
...
void			update_loading_bar(t_loadingbar *lb)
{
	...
	if (lb->val >= lb->old + LB)
	{
		...
		SDL_RenderCopy(get_sdl_core()->renderer,lb->loadingtexture, NULL, NULL);
